...
function AddInstrument(vsti, track_name)
	...
	local function ShowVSTi(track, slot, auto_float)
		...
		-- Position FX window at the middle of the screen and click on title bar in order to focus
		...
		if hwnd then
			...
			if got_val_ok then -- if retrieving the plugin size values did work
				...
				local _, left, top, right, bottom = reaper.JS_Window_GetClientRect(hwnd) -- get size coordinates of plugin window
				reaper.JS_Mouse_SetPosition(left, top) -- position mouse at the upper left of the plugin window
				local hit = reaper.JS_WindowMessage_Send(hwnd, "W_NCHITTEST", 0, 0, left, top)
				reaper.JS_WindowMessage_Send(hwnd, "WM_NCLBUTTONDOWN", hit, 0, left, top)
				reaper.JS_WindowMessage_Send(hwnd, "WM_NCLBUTTONUP", hit, 0, left, top)
				-- reaper.Main_OnCommand(reaper.NamedCommandLookup("_S&M_MOUSE_L_CLICK"), -1) -- execute left click to activate the window

				-- local x = math.ceil(right - ((right - left) / 2)) -- get center of the x-axis
				-- local y = math.ceil(bottom - ((bottom - top) / 2)) -- get center of the y-axis
				-- reaper.JS_Mouse_SetPosition(x, y) -- move cursor to the center of the plugin window
			end
		end
	end
	...
	local function Main()
		...
		if not reaper.JS_Window_GetForeground then
			reaper.ShowMessageBox(
				"This script requires an up-to-date version of the js_ReaScriptAPI extension."
		...
		if reaper.CountSelectedTracks(0) == 0 then
			...
			for i = 0, reaper.CountSelectedTracks(0) - 1 do -- loop through all selected tracks
				...
				local slot = AddVSTi(track) -- add VSTi and get VSTi slot
