-- @description Toggle show UI of all FX in 4th send slot
-- @version 1.0
-- @changelog
--   initial release
...
-- @about
--   # Description
--   - this script shows/hides the UI of all FX in the 4th send FX slot
...
elseif selected_tracks > 1 then
    ...
    local send_track = reaper.BR_GetMediaTrackSendInfo_Track(track, 0, 3, 1) -- get 4th send track
    
    reaper.Undo_BeginBlock()

    if reaper.GetTrackNumSends(track, 0) > 3 then
        ...
        reaper.ShowMessageBox("The selected track has no send FX in slot 4", "Error", 0)
        return false
    end

    reaper.Undo_EndBlock("Toggle show UI of 4th send FX", -1)
end
