-- @description Toggle show UI of all FX in 2nd send slot
-- @version 1.0
-- @changelog
--   initial release
...
-- @about
--   # Description
--   - this script shows/hides the UI of all FX in the 2nd send FX slot
...
elseif selected_tracks > 1 then
    ...
    local send_track = reaper.BR_GetMediaTrackSendInfo_Track(track, 0, 1, 1) -- get 2nd send track
    ...
    if reaper.GetTrackNumSends(track, 0) > 1 then
        ...
        reaper.ShowMessageBox("The selected track has no send FX in slot 2", "Error", 0)
        return false
    end

    reaper.Undo_EndBlock("Toggle show UI of 2nd send FX", -1)
end
