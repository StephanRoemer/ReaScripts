-- @description Toggle show UI of all FX in 1st send slot
-- @version 1.0
-- @changelog
--   initial release
...
-- @about
--   # Description
--   - this script shows/hides the UI of all FX in the 1st send FX slot
...
elseif selected_tracks > 1 then
    ...
    local send_track = reaper.BR_GetMediaTrackSendInfo_Track(track, 0, 0, 1) -- get 1st send track
    ...
    if reaper.GetTrackNumSends(track, 0) > 0 then
        ...
        reaper.ShowMessageBox("The selected track has no send FX in slot 1", "Error", 0)
        return false
    end

    reaper.Undo_EndBlock("Toggle show UI of 1st send FX", -1)
end
