-- @description Select all events in CC lane after and under mouse cursor
-- @version 1.0
-- @changelog
--   initial release
...
--    * When a CC selection already exists, the new selection will be added
...
function SelectCCAfterEditCursor(cc_lane)
    ...
    if reaper.TakeIsMIDI(take) then
        ...
        while stringPos < MIDIlen-12 do -- parse through all events in the MIDI string, one-by-one, excluding the final 12 bytes, which provides REAPER's All-notes-off end-of-take message
            ...
            and event_start > mouse_pos_ppq -- events are after cursor position
            then
                flags = flags|1 -- select muted and unmuted CC events
            elseif msg:byte(2) == cc_lane and event_start < mouse_pos_ppq then
...
SelectCCAfterEditCursor(cc_lane)

reaper.UpdateArrange()
reaper.Undo_OnStateChange2(proj, "Select all events in CC"..cc_lane.." lane after and under mouse cursor")