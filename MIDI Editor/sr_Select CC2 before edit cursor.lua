-- @description sr_Select CC2 before edit cursor
-- @version 1.0
    ...
    for t = 0, reaper.CountTakes(item)-1 do -- Loop through all takes within each selected item
        ...
        if reaper.TakeIsMIDI(take) then -- make sure, that take is MIDI
            ...
            for c = 0, ccCount - 1 do -- loop thru all CCs, back to forth
                ...
                if cc == 2 and ppqposOut < cursor_position_ppq then -- if CC is CC2
					reaper.MIDI_SetCC(take, c, true, nil, nil, nil, nil, nil, nil, true) -- select CC2
                elseif cc == 2 then -- if CC2 is beyond edit cursor
					reaper.MIDI_SetCC(take, c, false, nil, nil, nil, nil, nil, nil, true) -- unselect CC2
				end
            end
        end
    end
