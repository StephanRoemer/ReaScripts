-- @description Select all CCs before mouse cursor (in take under mouse or in MIDI editor)
-- @version 1.0
-- @changelog
--  * initial release
...
-- @about
--    # Description
--    * This script selects all CCs before the mouse cursor in the currently focused MIDI editor or inline editor.
...
local function SelectAllCCsBeforeMouse(take, item, mouse_pos)
    ...
    while string_pos < midi_len-12 do -- parse through all events in the MIDI string, one-by-one, excluding the final 12 bytes, which provides REAPER's All-notes-off end-of-take message
        ...
        and event_start <= mouse_position_ppq -- events are before cursor position
        then
...
if window == "midi_editor" and not inline_editor then -- MIDI editor focused and not hovering inline editor
    ...
    SelectAllCCsBeforeMouse(take, item, mouse_pos) -- execute function
...
reaper.Undo_OnStateChange2(proj, "Select all CCs before mouse cursor (in take under mouse or in MIDI editor)")