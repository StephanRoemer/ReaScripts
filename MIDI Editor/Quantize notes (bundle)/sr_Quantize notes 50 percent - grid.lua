...
local undo_text = "Quantize notes 50% - grid"
...
local function Main()
    ...
    local take, item, item_cnt, selnotes_items, save_proj_grid, save_proj_swing, save_proj_swing_amt, grid_midi, swing_midi, grid_minimum, grid_linked, grid_min_changed, midi_editor
    local window, _, _ = reaper.BR_GetMouseCursorContext() -- initialize cursor context
    local _, inline_editor, _, _, _, _ = reaper.BR_GetMouseCursorContext_MIDI() -- check if mouse hovers an inline editor

    item_cnt = reaper.CountSelectedMediaItems(0) -- count selected items
