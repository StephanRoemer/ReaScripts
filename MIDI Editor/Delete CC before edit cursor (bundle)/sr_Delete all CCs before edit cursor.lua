...
local function DeleteAllCCsBeforeEditCursor(take, item, cursor_position_ppq)
	...
	while string_pos < midi_len-12 do -- parse through all events in the MIDI string, one-by-one, excluding the final 12 bytes, which provides REAPER's All-notes-off end-of-take message
		...
		and event_start <= cursor_position_ppq -- and events are after cursor position
		then
...
if window == "midi_editor" then -- MIDI editor focused
	...
	DeleteAllCCsBeforeEditCursor(take, item, cursor_position_ppq) -- delete all CCs before cursor
