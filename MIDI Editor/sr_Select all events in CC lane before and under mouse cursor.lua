-- @description Select all events in CC lane before and under mouse cursor
-- @version 1.0
-- @changelog
--   initial release
...
function SelectCCBeforeMouse(cc_lane)
    ...
    if reaper.TakeIsMIDI(take) then
        ...
        while stringPos < MIDIlen-12 do -- parse through all events in the MIDI string, one-by-one, excluding the final 12 bytes, which provides REAPER's All-notes-off end-of-take message
            ...
            then
                if event_start < mouse_pos_ppq then -- events are before mouse position
                    flags = flags|1 -- select muted and unmuted CC events
                elseif event_start > mouse_pos_ppq then
                    flags = flags&0 -- unselect muted and unmuted CC events after mouse	
                end
            end
...
reaper.Undo_OnStateChange2(proj, "Select all events in CC"..cc_lane.." lane before and under mouse cursor")