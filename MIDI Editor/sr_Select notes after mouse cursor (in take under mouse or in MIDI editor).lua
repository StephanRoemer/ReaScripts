-- @description Select notes after mouse cursor (in take under mouse or in MIDI editor)
-- @version 1.01
-- @changelog
...
local function SelectNotesAfterMouse(take, item, mouse_pos)
    ...
    while string_pos < midi_len-12 do -- parse through all events in the MIDI string, one-by-one, excluding the final 12 bytes, which provides REAPER's All-notes-off end-of-take message
        ...
        if event_type == 9 and msg:byte(3) ~= 0 then -- if note-on and velocity is not 0
            ...
            -- note-on after mouse position? select	
            ...
            -- note-on before mouse position? unselect 
            elseif event_start <= mouse_position_ppq then 
        ...
        elseif event_type == 8 or (event_type == 9 and msg:byte(3) == 0) then -- if note-off
            ...
            -- note-off anywhere and note-on after mouse? select
            ...
            -- note-off and note-on before mouse? unselect
            else
                flags = flags&~1 -- unselect
            end
        end
...
if window == "midi_editor" and not inline_editor then -- MIDI editor focused and not hovering inline editor
    ...
    SelectNotesAfterMouse(take, item, mouse_pos) -- execute function
...
reaper.Undo_OnStateChange2(proj, "Select notes after mouse cursor (in take under mouse or in MIDI editor)")