...
-- quantize open take in MIDI editor
        ...
        if notes_selected == true then -- if there is a note selection
            if selected_out == true then -- filter out selected notes to quantize
                if closest_grid_ppq ~= startppqpos_out then -- if notes are not on the grid
                    reaper.MIDI_SetNote(take, n, nil, nil, startppqpos_out+(closest_grid_ppq-startppqpos_out)*humanize/100, startppqpos_out+(closest_grid_ppq-startppqpos_out)*humanize/100+endppqpos_out-startppqpos_out, nil, nil, nil, true) -- quantize selected notes
                end
            end
