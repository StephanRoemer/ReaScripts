...
function SelectCC(cc_lane, take) -- select or unselect CC
    ...
    while stringPos < MIDIlen-12 do -- parse through all events in the MIDI string, one-by-one, excluding the final 12 bytes, which provides REAPER's All-notes-off end-of-take message
        ...
        and (msg:byte(1)>>4) == 11 then -- if status byte is a CC
            if msg:byte(2) == cc_lane then -- CC# equals cc_lane
                flags = flags|1 -- select muted or unmuted event
            elseif msg:byte(2) ~= cc_lane then -- CC# does not equal cc_lane
                flags = flags&0 -- unselect all other muted or unmuted CC events
            end
        end
...
if reaper.TakeIsMIDI(take) then -- make sure, that take is MIDI
    ...
    SelectCC(cc_lane, take) -- select or unselect CC events
end
