...
-- change start position of selected notes
	...
	for n = 0, notesCount - 1 do
		...
		if selectedOut == true then -- is current note selected?
			if val > 0 then -- if mousewheel up
				reaper.MIDI_SetNote(take, n, nil, nil, startppqposOut+val, nil, nil, nil, nil, true) -- increase note start by val
			else 			-- if mousewheel down
				reaper.MIDI_SetNote(take, n, nil, nil, startppqposOut+val, nil, nil, nil, nil, true) -- decrease note start by val
			end
		end
	end

-- if there are no selected notes, only change the start of the note under the mouse cursor
else 
	for n = 0, notesCount - 1 do
...
reaper.Undo_EndBlock("Change note start (mousewheel)", -1)
