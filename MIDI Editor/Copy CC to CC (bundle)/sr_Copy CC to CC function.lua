...
function CopySrcCCToDestCC(src_cc, dest_cc)
	...
	-- copy src_cc to dest_cc in MIDI/inline Editor (respect event selection)

	local function CopyCCMIDIEditor(take)
		...
		-- second iteration, copy src_cc to dest_cc
		...
		while string_pos < midi_len-12 do -- parse through all events in the MIDI string, one-by-one, excluding the final 12 bytes, which provides REAPER's All-notes-off end-of-take message
			...
			and (flags&1 == 1 or not src_cc_selected) -- selected events always get copied, unselected only get copied if no notes are selected
			then
				table.insert(table_events, string.pack("i4Bs4", offset, flags, msg)) -- re-pack MIDI string and write original src_cc
				msg = msg:sub(1,1) .. string.char(dest_cc) .. msg:sub(3,3) -- copy src_cc events to dest_cc
				table.insert(table_events, string.pack("i4Bs4", 0, flags, msg)) -- re-pack MIDI string with dest_cc (offset 0)
			else
				table.insert(table_events, string.pack("i4Bs4", offset, flags, msg)) -- re-pack MIDI string with table_eventsd events
			end
		end
	...
	-- copy src_cc to dest_cc in arrange view (ignore event selection)

	local function CopyCCArrange(take)
