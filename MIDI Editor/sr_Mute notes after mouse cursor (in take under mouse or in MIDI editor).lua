-- @description Mute notes after mouse (in take under mouse or in MIDI editor)
-- @version 1.0
-- @changelog
--  * initial release
...
-- @about
--    # Description
--    * This script mutes all notes after the mouse cursor in the take that the mouse is currently hovering (arrangement and inline editor) or in the focused MIDI editor
    ...
    while stringPos < MIDIlen-12 do -- parse through all events in the MIDI string, one-by-one, excluding the final 12 bytes, which provides REAPER's All-notes-off end-of-take message
        ...
        if event_type == 9 and msg:byte(3) ~= 0 then -- if note-on and velocity is not 0
            ...
            elseif event_start > mouse_position_ppq then
                flags = flags|2 -- mute
                note_on_selection[channel][flags&2][pitch] = true -- tag note-on for mute

            -- note-on before mouse position? unmute 
            elseif event_start <= mouse_position_ppq then 
                flags = flags&~2 -- unmute
                note_on_selection[channel][flags&2][pitch] = nil -- tag note-on for non-mute
            end
        ...
        elseif event_type == 8 or (event_type == 9 and msg:byte(3) == 0) then -- if note-off
            ...
            if note_on_selection[channel][2][pitch] then -- matching note-on tagged for mute?
                flags = flags|2 -- mute
...
reaper.Undo_OnStateChange2(proj, "Mute notes after mouse (in take under mouse or in MIDI editor)")