-- @description Move selected CC events to lane under mouse cursor at mouse position
-- @version 1.0
-- @changelog
--   initial release
...
local function MoveCCToLaneUnderMouseAtMouse(cc_lane, take)
    ...
    while stringPos < MIDIlen-12 do -- parse through all events in the MIDI string, one-by-one, excluding the final 12 bytes, which provides REAPER's All-notes-off end-of-take message
        ...
        then
            ...
            table.insert(tableEvents, string.pack("i4Bs4", offset, flags, "")) -- delete original CC event
            msg_cc_lane = msg:sub(1,1) .. string.char(cc_lane) .. msg:sub(3,3) -- write msg chunk for cc_lane
            table.insert(tableEvents, string.pack("i4Bs4", mouse_pos_ppq_int-sum_offset, flags, msg_cc_lane)) -- move CC event to mouse cursor and re-pack MIDI string 
...
if reaper.TakeIsMIDI(take) then -- make sure, that take is MIDI
    ...
    MoveCCToLaneUnderMouseAtMouse(cc_lane, take) -- move CC events to cc_lane
end

reaper.UpdateArrange()
reaper.Undo_OnStateChange2(proj, "Move selected CC events to CC"..cc_lane.." lane under mouse cursor at mouse position")