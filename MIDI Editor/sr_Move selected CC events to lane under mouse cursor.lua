-- @description Move selected CC events to lane under mouse cursor
-- @version 1.0
-- @changelog
--   initial release
...
selection_offset = false

local function MoveCCToLaneUnderMouse(cc_lane, take)
    ...
    while stringPos < MIDIlen-12 do -- parse through all events in the MIDI string, one-by-one, excluding the final 12 bytes, which provides REAPER's All-notes-off end-of-take message
        ...
        then
            ...
            table.insert(tableEvents, string.pack("i4Bs4", 0, flags, msg_cc_lane)) -- re-pack MIDI string and write copied CC events to cc_lane
        else
...
if reaper.TakeIsMIDI(take) then -- make sure, that take is MIDI
    ...
    MoveCCToLaneUnderMouse(cc_lane, take) -- move CC events to cc_lane
end

reaper.UpdateArrange()
reaper.Undo_OnStateChange2(proj, "Move selected CC events to CC"..cc_lane.." lane under mouse cursor")