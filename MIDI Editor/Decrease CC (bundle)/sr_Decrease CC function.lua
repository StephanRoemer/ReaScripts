...
function DecreaseCC(dest_cc, decrease)
	

	-- decrease dest_cc in MIDI/inline Editor (respect event selection)

	local function DecreaseCCMIDIEditor(take)
		...
		-- second iteration, decrease dest_cc
		...
		while string_pos < midi_len-12 do -- parse through all events in the MIDI string, one-by-one, excluding the final 12 bytes, which provides REAPER's All-notes-off end-of-take message
			...
			then
				...
				msg = msg:sub(1,1) .. msg:sub(2,2) .. string.char(math.min(127, (math.ceil(msg_b3/decrease)))) -- decrease CC value, convert CC value to string, concatenate msg
			end
	...
	-- decrease dest_cc in arrange view (ignore event selection)

	local function DecreaseCCArrange(take)
	...
	local take, item, dest_cc, decrease
	...
	if window == "midi_editor" then -- MIDI editor focused
		...
		DecreaseCCMIDIEditor(take) -- decrease CC
		...
		if reaper.CountSelectedMediaItems(0) ~= 0 then
			for i = 0, reaper.CountSelectedMediaItems(0)-1 do -- loop through all selected items
				...
				if reaper.TakeIsMIDI(take) then -- make sure, that take is MIDI
					...
					DecreaseCCArrange(take) -- decrease CC
