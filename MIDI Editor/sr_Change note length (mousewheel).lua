	...
	if notes_selected == true then -- if there are selected notes
		if selected_out == true then -- is current note selected?
			if val > 0 then -- if mousewheel up
				reaper.MIDI_SetNote(take, n, nil, nil, nil, endppqpos_out+val, nil, nil, nil, true) -- increase note length by val
			else 			-- if mousewheel down
				reaper.MIDI_SetNote(take, n, nil, nil, nil, endppqpos_out+val, nil, nil, nil, true) -- decrease note length by val
			end
		end
	else -- if there are no selected notes, only change the length of the note under the mouse cursor
