-- @description Select notes and all CCs after edit cursor (in selected item(s) or MIDI editor)
-- @version 1.0
-- @changelog
--  + initial release
...
-- @about
--    # Description
--    * This script selects all notes and CCs after the edit cursor in currently selected items or in the currently opened take in the MIDI editor.
...
local function SelectNotesAndCCsAfterCursor(take, item, cursor_position_ppq)
	...
	while string_pos < midi_len-12 do -- parse through all events in the MIDI string, one-by-one, excluding the final 12 bytes, which provides REAPER's All-notes-off end-of-take message
		...
		if event_type == 9 and msg:byte(3) ~= 0 then -- if note-on and velocity is not 0
			...
			elseif event_start >= cursor_position_ppq then
				...
				note_on_tagging[channel][flags&2][pitch] = true -- tag note-on for selection

			-- note-on before cursor position? unselect 
			elseif event_start <= cursor_position_ppq then 
				flags = flags &~ 1 -- unselect
				note_on_tagging[channel][flags&2][pitch] = nil -- tag note-on for non-selection
			end
		...
		elseif event_type == 8 or (event_type == 9 and msg:byte(3) == 0) then --  if note-off or velocity 0
			...
			if note_on_tagging[channel][flags&2][pitch] then -- matching note-on tagged for selection?
				flags = flags|1 -- select
		...
		-- select all CCs after edit cursor
		elseif #msg == 3 and event_type == 11  then -- if msg consists of 3 bytes (= channel message) and status byte is a CC
			if event_start >= cursor_position_ppq then -- events are after cursor position
...
if window == "midi_editor" then -- MIDI editor focused
	...
	SelectNotesAndCCsAfterCursor(take, item, cursor_position_ppq) -- select notes and all CCs
...
reaper.Undo_OnStateChange2(proj, "Select notes and all CCs after edit cursor (in selected item(s) or MIDI editor)")