-- @description sr_Select CC64 before edit cursor
-- @version 1.0
-- @author Stephan Römer
-- @about
--    # Description
--    - this script selects the CC64 data before the edit cursor
...
for i = 0, reaper.CountSelectedMediaItems(0)-1 do -- loop through all selected items
    ...
    for t = 0, reaper.CountTakes(item)-1 do -- Loop through all takes within each selected item
        ...
        if reaper.TakeIsMIDI(take) then -- make sure, that take is MIDI
            ...
            for c = 0, ccCount - 1 do -- loop thru all CCs, back to forth
                ...
                if cc == 64 and ppqposOut < cursor_position_ppq then -- if CC is CC64
					reaper.MIDI_SetCC(take, c, true, nil, nil, nil, nil, nil, nil, true) -- select CC64
                elseif cc == 64 then -- if CC64 is beyond edit cursor
					reaper.MIDI_SetCC(take, c, false, nil, nil, nil, nil, nil, nil, true) -- unselect CC64
				end
            end
        end
    end
